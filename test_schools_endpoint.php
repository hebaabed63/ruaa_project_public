...
// Simple test using cURL to check the schools endpoint
function testSchoolsEndpoint() {
    ...
    if ($loginResponse['status'] == 200) {
        ...
        if (isset($loginResult['data']['token'])) {
            ...
            $schoolsResponse = makeRequest('GET', 'http://127.0.0.1:8000/api/admin/schools', [], [
                'Authorization: Bearer ' . $token
            ]);
            
            echo "Schools Status: " . $schoolsResponse['status'] . "\n";
            echo "Schools Response: " . $schoolsResponse['body'] . "\n";
        } else {
