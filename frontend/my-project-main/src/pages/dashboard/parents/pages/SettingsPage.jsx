...
const SettingsPage = () => {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      await parentsAPI.changePassword({
        currentPassword: tempSettings.currentPassword,
        newPassword: tempSettings.newPassword,
        confirmPassword: tempSettings.confirmPassword,
      });
      ...
      if (error.response?.data?.errors) {
        ...
        if (errors.current_password) {
          backendErrors.currentPassword = errors.current_password[0];
        }
        if (errors.new_password) {
          backendErrors.newPassword = errors.new_password[0];
        }
        if (errors.confirm_password) {
          backendErrors.confirmPassword = errors.confirm_password[0];
        }
        
        setErrors(backendErrors);
      } else {
        setErrors({ general: error.message || 'حدث خطأ أثناء تغيير كلمة المرور' });
      }
    } finally {
      setIsSaving(false);
    }
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      if (activeTab === 'notifications') {
        await parentsAPI.updateNotificationSettings({
          email_notifications: tempSettings.emailNotifications,
          push_notifications: tempSettings.desktopNotifications,
          sms_notifications: tempSettings.smsNotifications,
        });
      }
      
      // Update local UI settings (for privacy and other UI-only settings)
      ...
      setErrors({ general: error.message || 'حدث خطأ أثناء حفظ الإعدادات' });
    } finally {
      setIsSaving(false);
    }
  };
