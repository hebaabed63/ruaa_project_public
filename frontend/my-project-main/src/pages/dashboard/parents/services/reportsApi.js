...
// =============================================================================
// Real Backend Integration for Student Reports
// ربط حقيقي مع الباك إند لتقارير الطلاب
// =============================================================================


/**
 * Get student report for a specific child and term from real backend
 * جلب تقرير طالب لطفل محدد وفصل دراسي محدد من الباك إند
 */
export const getStudentReport = async (childId, term) => {
  try {
    const response = await apiAxios.get('/parent/reports', {
      params: {
        child_id: childId,
        term: term
      }
    ...
    return {
      success: false,
      message: response.data.message || 'فشل في جلب التقرير'
    };
  } catch (error) {
    console.error('Error fetching student report:', error);
    
    const errorMessage = error.response?.data?.message || 
                        error.response?.data?.errors?.child_id?.[0] ||
                        error.response?.data?.errors?.term?.[0] ||
...
/**
 * Get available terms for a child
 * جلب الفصول الدراسية المتاحة لطفل
 */
export const getAvailableTerms = async () => {
  // For now, return static terms - can be made dynamic later
  return {
    success: true,
    data: [
      { value: '2024-2025-1', label: 'الفصل الأول 2024-2025' },
      { value: '2024-2025-2', label: 'الفصل الثاني 2024-2025' },
      { value: '2023-2024-2', label: 'الفصل الثاني 2023-2024' },
      { value: '2023-2024-1', label: 'الفصل الأول 2023-2024' },
    ]
  };
};
 ...
 */
export const exportReportPDF = async (childId, term) => {
  try {
    const response = await apiAxios.get('/parent/reports/export', {
      params: { child_id: childId, term: term },
      responseType: 'blob'
    });
    ...
    link.href = url;
    link.setAttribute('download', `report-${childId}-${term}.pdf`);
