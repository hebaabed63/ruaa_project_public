...
import { useParentProfileContext } from '../../contexts/ParentProfileContext';
          ...
          >
            {profile?.profileImage ? (
              ...
              <div className="w-8 h-8 rounded-full bg-black flex items-center justify-center text-white text-sm font-bold">
                {profile?.fullName?.charAt(0) || profile?.name?.charAt(0) || 'و'}
              </div>
            )}
            <div className="flex flex-col items-start text-left">
              <span className="text-black dark:text-gray-300 font-medium truncate">
                {profileLoading ? '...' : (profile?.fullName?.split(' ')[0] || profile?.name?.split(' ')[0] || 'ولي أمر')}
              </span>
                  ...
                  <div className="flex items-center space-x-3 space-x-reverse">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 dark:text-white truncate">
                        {profile?.fullName || profile?.name || 'ولي الأمر'}
                      </p>
