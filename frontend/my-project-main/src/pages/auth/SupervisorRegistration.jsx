...
import { validateRegistrationToken, registerSupervisor } from "../../services/authService";
...
// Validation schema for supervisor registration
...
export default function SupervisorRegistration() {
  ...
  useEffect(() => {
    const validateToken = async () => {
      ...
      try {
        ...
        if (response.success && response.data.link_type === 'supervisor') {
          setTokenData(response.data);
        } else {
          setError('رابط التسجيل غير صالح للمشرفين');
        }
      } catch (err) {
  ...
  const handleSubmit = async (values, { setSubmitting, setFieldError }) => {
    ...
    try {
      ...
      const response = await registerSupervisor({
        token,
      ...
      if (response.success) {
        ...
        showAlert('success', 'تم التسجيل بنجاح', 'جاري تحويلك إلى لوحة التحكم...');
  ...
  const handleGoogleRegister = () => {
    ...
    window.location.href = `${process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000'}/api/auth/google?registration_token=${token}&role=supervisor`;
  };
  ...
  return (
    ...
    >
      <div className="mt-4">
        {/* Organization Info - Made linear to save space */}
        {tokenData && tokenData.organization_name && (
          <div className="mb-4 p-2 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center justify-center space-x-1 space-x-reverse">
              <FaUserTie className="text-green-600 text-sm" />
              <span className="text-xs text-green-800">تسجيل تحت مظلة:</span>
              <span className="text-xs font-medium text-green-600">{tokenData.organization_name}</span>
            </div>
          </div>
        )}
            ...
            <Form className="space-y-3 w-full">
              ...
              <div className="flex items-center gap-1.5 font-cairo font-normal px-2 mb-3 text-xs text-black">
                ...
                <span className="text-xs">
                  أوافق على{" "}
                  <Link to="/terms/supervisor" className="text-blue-600 underline text-xs">
                    شروط الاستخدام
                  </Link>{" "}
                  و{" "}
                  <Link to="/privacy/supervisor" className="text-blue-600 underline text-xs">
                    سياسة الخصوصية
                  </Link>
                </span>
              </div>
              ...
              <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="text-sm text-yellow-800">
                  <p className="font-medium">ملاحظة: سيتم مراجعة طلبك من قبل الإدارة خلال 1-3 أيام عمل</p>
                </div>
              </div>
            </Form>
          );
