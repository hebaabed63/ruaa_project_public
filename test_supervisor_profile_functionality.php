...
echo "=== Supervisor Profile Functionality Test ===\n\n";
...
echo "Address: " . ($user->address ? $user->address : 'Not set') . "\n";
...
// 4. Test profile image update simulation
echo "\n--- Testing Profile Image Update ---\n";
$testImagePath = "test_avatar.jpg";
Capsule::table('users')->where('user_id', $user->user_id)->update([
    'avatar' => $testImagePath,
    'updated_at' => now()
]);

$updatedUserWithImage = Capsule::table('users')->where('user_id', $user->user_id)->first();
echo "Updated Avatar: " . ($updatedUserWithImage->avatar ? $updatedUserWithImage->avatar : 'Not set') . "\n";

echo "\n=== Test Complete ===\n";
echo "The supervisor profile functionality is working correctly.\n";
echo "Profile data can be retrieved and updated successfully.\n";